...
function showSmer() {
  ...
  // Funkcija koja će se izvršiti nakon klika na dugme "Potvrdi"
    document.getElementById("my-form").addEventListener("submit", function(event) {
    event.preventDefault(); // sprečava da se forma automatski pošalje
  
    // Dobijanje vrednosti polja forme
    var ime = document.getElementsByName("ime")[0].value;
    var prezime = document.getElementsByName("prezime")[0].value;
    var potpis = document.getElementsByName("potpis")[0].value;
  
    // Kreiranje URL adrese za novu stranicu sa parametrima
    var url = "prikaz-podataka.html?ime=" + ime + "&prezime=" + prezime + "&potpis=" + potpis;
  
    // Otvoriti novu stranicu sa podacima koje je korisnik uneo
